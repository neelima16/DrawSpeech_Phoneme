...
def get_preprocessed_meta(config, dl_output, metadata):
    ...
    if "pitch_sketch" in metadata.keys() and metadata["pitch_sketch"]:  # under inference mode
        ...
        pitch_sketch = F.pad(pitch_sketch, (0, pitch_pad_length - pitch_sketch.size(0)), value=0) if pitch_sketch.size(0) < pitch_pad_length else pitch_sketch[:pitch_pad_length]
    ...
    if "energy_sketch" in metadata.keys() and metadata["energy_sketch"]:  # under inference mode
        ...
        energy_sketch = sketch_extractor(energy_sketch)
        ...
        energy_sketch = F.pad(energy_sketch, (0, energy_pad_length - energy_sketch.size(0)), value=0) if energy_sketch.size(0) < energy_pad_length else energy_sketch[:energy_pad_length]
